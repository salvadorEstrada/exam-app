...
export class AddPreguntaComponent implements OnInit { 
  ...
  ngOnInit(): void { 
    this.examenId=this.route.snapshot.params['examenId'];//El examenId se está obteniendo de la ruta
  ...
  formSubmit(){
    ...
    this.preguntaService.guardarPregunta(this.pregunta).subscribe(
      (data)=>{
        Swal.fire('Pregunta guardada','La pregunta ha sido agregada con exito!!','success'); 
      ...
      (error)=>{
        Swal.fire('Error','Error al guardar la pregungta en la base de datos','error');
        console.log(error);
      }
    )
